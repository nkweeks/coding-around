/* ============================================================
 * VIM Protocol 3 — Vim engine (evolved from the v2 engine)
...
(function () {
  ...
  VimEngine.prototype.line = function (r) {
    return this.lines[r === undefined ? this.row : r];
  };
  VimEngine.prototype.lastRow = function () {
  ...
  VimEngine.prototype.matchBracket = function () {
    ...
    for (var i = this.col; i < line.length; i++) {
      if (BRACKETS[line[i]] || BRACKETS_R[line[i]]) {
        c0 = i;
        ch = line[i];
        break;
      }
  ...
  VimEngine.prototype.findCharCol = function (type, ch, fromColOverride) {
    ...
    if (type === "f" || type === "t") {
      ...
      var from2 =
        fromColOverride !== undefined ? fromColOverride : this.col - 1;
  ...
  VimEngine.prototype.findMatch = function (pat, dir) {
    ...
    if (dir === 1) {
      ...
      var idx2 =
        this.col > 0 ? this.lines[this.row].lastIndexOf(pat, this.col - 1) : -1;
  ...
  VimEngine.prototype.textObject = function (objChar, around) {
    ...
    if (objChar === "w") {
      ...
      if (around) {
        ...
        if (e2 === e) {
          // no trailing space: take leading
    ...
    if (objChar === '"' || objChar === "'" || objChar === "`") {
      ...
      for (var i = 0; i < line.length; i++)
        if (line[i] === objChar) positions.push(i);
      for (var k = 0; k + 1 < positions.length; k += 2) {
        var openQ = positions[k],
          closeQ = positions[k + 1];
        if (closeQ >= col || openQ >= col) {
          if (around)
            return {
              start: { row: row, col: openQ },
              end: { row: row, col: closeQ + 1 },
            };
          return {
            start: { row: row, col: openQ + 1 },
            end: { row: row, col: closeQ },
          };
        }
      }
      return null;
  ...
  VimEngine.prototype.operateRange = function (op, start, end) {
    ...
    if (op === "y") {
      if (!empty)
        this.register = { pieces: this.getRange(start, end), linewise: false };
  ...
  VimEngine.prototype.insertPiecesAt = function (pos, pieces) {
    ...
    if (pieces.length === 1) {
      this.lines[pos.row] =
        line.slice(0, pos.col) + pieces[0] + line.slice(pos.col);
    ...
    for (var i = 0; i < mid.length; i++)
      this.lines.splice(insertAt + i, 0, mid[i]);
    this.lines.splice(insertAt + mid.length, 0, tail);
    return {
      row: insertAt + mid.length,
  ...
  VimEngine.prototype.insertKey = function (k) {
    ...
    if (k === "<Up>" || k === "<Down>") {
      this.row = Math.max(
        0,
        Math.min(this.lastRow(), this.row + (k === "<Up>" ? -1 : 1)),
      );
      this.col = Math.min(this.line().length, this.col);
      return;
    }
    if (k === "<Esc>") {
      this.mode = MODE.NORMAL;
    ...
    if (k === "<CR>") {
      ...
      var head = line.slice(0, this.col),
        tail = line.slice(this.col);
  ...
  VimEngine.prototype.handlePendingChar = function (k) {
    ...
    if (pc.type === "obj") {
      if (!this.op && this.mode === MODE.NORMAL) {
        this.resetPending();
        return;
      }
      ...
      var op = this.op || "d";
      if (this.mode === MODE.VISUAL || this.mode === MODE.VLINE) {
        this.resetPending();
        return;
      }
    ...
    if (pc.type === "r") {
      ...
      if (line.length > 0) {
        this.beginChange();
        this.lines[this.row] =
          line.slice(0, this.col) + k + line.slice(this.col + 1);
      }
      this.resetPending();
      return;
    }
  ...
  VimEngine.prototype.visualRange = function () {
    var a = this.visual,
      b = { row: this.row, col: this.col };
    if (this.mode === MODE.VLINE) {
      return {
        lo: Math.min(a.row, b.row),
        hi: Math.max(a.row, b.row),
        linewise: true,
      };
    }
  ...
  VimEngine.prototype.visualOperate = function (op) {
    ...
    if (wasVline) {
      ...
      var end = {
        row: vr.end.row,
        col: Math.min(vr.end.col, this.lines[vr.end.row].length),
      };
  ...
  VimEngine.prototype.motionFor = function (k) {
    ...
    switch (k) {
      case "h":
        return { row: row, col: Math.max(0, col - n), kind: "e" };
      case "l":
      case " ":
        return {
          row: row,
          col: Math.min(Math.max(0, line.length - (this.op ? 0 : 1)), col + n),
          kind: "e",
        };
      case "j":
        return { row: Math.min(this.lastRow(), row + n), col: col, kind: "l" };
      case "k":
        return { row: Math.max(0, row - n), col: col, kind: "l" };
      case "0":
        return { row: row, col: 0, kind: "e" };
      case "^":
        return { row: row, col: this.firstNonBlank(row), kind: "e" };
      case "$": {
        var r = Math.min(this.lastRow(), row + n - 1);
        return {
          row: r,
          col: Math.max(0, this.lines[r].length - (this.op ? 0 : 1)),
          kind: this.op ? "e" : "i",
        };
      }
      case "w": {
      ...
      case "G": {
        var gr =
          this.count !== "" || this.opCount !== "" ? n - 1 : this.lastRow();
        gr = Math.min(gr, this.lastRow());
        return { row: gr, col: this.firstNonBlank(gr), kind: "l" };
      }
      case "{": {
        var t1 = this.paraBack();
        return { row: t1.row, col: t1.col, kind: "e" };
      }
      case "}": {
        var t2 = this.paraForward();
        return { row: t2.row, col: t2.col, kind: "e" };
      }
      case "%": {
  ...
  VimEngine.prototype.normalKey = function (k) {
    ...
    if (this.gPending) {
      ...
      if (k === "g") {
        var gr =
          this.count !== "" || this.opCount !== "" ? this.totalCount() - 1 : 0;
        gr = Math.min(gr, this.lastRow());
        this.applyMotionResult(
          { row: gr, col: this.firstNonBlank(gr), kind: "l" },
          "gg",
        );
        this.count = "";
        this.opCount = "";
    ...
    if (/^[0-9]$/.test(k)) {
      ...
      if (!(k === "0" && buf === "")) {
        if (this.op) this.opCount = (this.opCount + k).slice(0, 4);
        else this.count = (this.count + k).slice(0, 4);
        return;
      }
    }
    ...
    if (this.op && k === this.op) {
      ...
      this.operateLinewise(
        this.op,
        this.row,
        Math.min(this.lastRow(), this.row + nLines - 1),
      );
      this.resetPending();
      return;
    }
    ...
    switch (k) {
      ...
      case "Y": {
        this.operateLinewise(
          "y",
          this.row,
          Math.min(this.lastRow(), this.row + n - 1),
        );
        this.count = "";
        return;
      }
      ...
      case "J": {
        ...
        for (var ji = 0; ji < joins; ji++) {
          ...
          var joined =
            cur.length === 0 ? nxt : cur + (nxt.length ? " " + nxt : "");
  ...
  VimEngine.prototype.key = function (k) {
    ...
    if (!this.replaying) {
      ...
      if (
        idleAtEnd &&
        this.changeTick > this._cmdStartTick &&
        !this._cmdNonRepeat &&
        this._cmdKeys.length
      ) {
  ...
  VimEngine.prototype.pendingDisplay = function () {
    ...
    if (this.pendingChar) {
      if (this.pendingChar.type === "obj")
        s += this.pendingChar.around ? "a" : "i";
  ...
  VimEngine.prototype.getState = function () {
    ...
    return {
      ...
      search:
        this.mode === MODE.SEARCH
          ? (this.searchDir === 1 ? "/" : "?") + this.searchBuf
          : null,
